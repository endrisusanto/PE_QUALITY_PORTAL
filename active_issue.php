    ...
    <div class="navbar-header">
	  ...
	  <li class="dropdown"><a class="dropdown-toggle thick" data-toggle="dropdown" href="#"><span class="glyphicon glyphicon-user"></span> Hi , <?php if( !isset($_SESSION['name']) ){    echo "Selamat Datang !" ;}   else{    echo $_SESSION['name']." [".$_SESSION['level']."]" ;}    ?>
        <ul class="dropdown-menu">
			...
			<li><a href="data_user.php"><span class="glyphicon glyphicon-list-alt"></span> SUMMARY USER</a></li>
...
if ($_SESSION['level']=='super user'){
	$query_mysql = mysqli_query($koneksi,"SELECT * FROM `analisa` WHERE status NOT LIKE 'Finish%' ORDER BY `analisa`.`week` DESC ");
}
else{
	$pengguna = $_SESSION['name'];
	$query_mysql = mysqli_query($koneksi,"SELECT * FROM `analisa` WHERE nama='$pengguna' AND status NOT LIKE 'Finish%' ORDER BY `analisa`.`week` DESC ");	
}
$nomor = 1;
